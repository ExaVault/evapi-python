        ...
        try:
            ...
            if method in ['POST', 'PUT', 'PATCH', 'OPTIONS', 'DELETE']:
                ...
                if re.search('json', headers['Content-Type'], re.IGNORECASE):
                    ...
                    # EV send files in url params instead of body
                    if post_params.get('filePaths') is not None:
                        encodedPaths = [];
                        tmpPaths = post_params.get('filePaths')
                        del post_params['filePaths']
                        for path in tmpPaths:
                            encodedPaths.append("filePaths[]="+urllib.quote_plus(path)) 
                        encodedPaths = '&'.join(encodedPaths)
                        url += "?" + encodedPaths
                        print(url)
